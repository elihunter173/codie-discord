...
impl Bot {
    ...
    pub async fn run_code(&self, lang: &str, code: &str) -> anyhow::Result<Output> {
        match lang {
            $($($lang)|+ => self.run($image, &$cmd, $path, code).await,)*
            _ => Err(crate::UserError(format!("I'm sorry, I don't know how to run {}", lang)).into()),
        }
    }

    pub fn help_lang(&self, lang: &str) -> Option<&str> {
        match lang {
            $($($lang)|+ => Some(concat!(
...
mod autogenerated_tests {
    ...
    $(
        paste::item! {
            #[tokio::test]
            async fn [<test_hello_world_ $name>]() {
                let output = RUNNER.run_code(first!($($lang),+), $hello_world).await.unwrap();
...
mod tests {
    ...
    async fn test_timeout() {
        assert!(matches!(
            RUNNER.run_code("python", "while True: pass").await,
            Err(_)
        ));
    }
...
sys.exit(123)
"#;
        let output = RUNNER.run_code("python", code).await.unwrap();
